...
// 映射）由 functionspacker 包的单元测试覆盖；packServer 与 Service 类型不
...
// functionspacker/fixture_test.go 同模式——测试夹具跨包不可导出，各自
...
func TestIntegration_GitSourceGoFunctionFullChainE2E(t *testing.T) {
	...
	packResp, err := functionspacker.PackGit(context.Background(),
		functionspacker.PackRequest{
			URL:       fx.fileURL(),
			Ref:       "", // HEAD
			Directory: "functions/greet",
		}, functionspacker.PackOptions{})
	require.NoError(t, err)
...
// 的 5000 条目上限由 functionspacker 单元测试覆盖）。
