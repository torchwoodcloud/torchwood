...
const RPC_TO_METHOD: Record<string, Record<string, string>> = {
  ...
  RunbookService: {
    GetRunbookState: "getState",
    RecordRunbookStep: "recordStep",
    DeleteRunbookStep: "deleteStep",
  },
  OAuthProvidersService: {
...
it("Torchwood.server 门面可达全部 Server swagger 服务（含 functions）", () => {
  ...
  it("写方法 HTTP 绑定（method/path/query/body 参数）与 swagger 一致", async () => {
    ...
    const server = (h: HttpTransport) => ({
      ...
      auth: new AuthService(h),
      runbooks: new RunbookService(h),
    ...
    cases.push(
      ...
      { side: "server", operationId: "APIKeysService_WhoAmI", invoke: (h) => server(h).apiKeys.whoAmI() },
      { side: "server", operationId: "AuthService_VerifyToken", invoke: (h) => server(h).auth.verifyToken("sk-token", "VERIFY_CREDENTIAL_TYPE_API_KEY") },
      { side: "server", operationId: "RunbookService_GetRunbookState", invoke: (h) => server(h).runbooks.getState("main") },
      { side: "server", operationId: "RunbookService_RecordRunbookStep", invoke: (h) => server(h).runbooks.recordStep("main", { version: 1, name: "init", checksum: "a".repeat(64), expect_prev_version: 0 }) },
      { side: "server", operationId: "RunbookService_DeleteRunbookStep", invoke: (h) => server(h).runbooks.deleteStep("main", 1) },
