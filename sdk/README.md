...
| Agent 工具 schema 来源 | **OpenAPI** | `mise run generate:proto` 后在 `genproto/**/*.swagger.json` 获取 |
| Agent 默认工具箱 | **E-7 overlay** | 18 个动词映射现有 Server RPC（`sdk/go/server/tools.go` / `agentTools`）；完整 API 仍是 188 RPC（Client 61 + Server 117 + Console 10），不含 API key 管理，见 `docs/developer/14-agent-tools.md` |
| 快速验证 | **Web 演示** | `mise run sdk:demo`，设置页填入 Console API Keys 页面创建的 API Key |
...
- SDK 测试通过 bufconn 内存 gRPC 服务运行，无需外部依赖；`mise run test` / `mise run lint:go` 已覆盖 `sdk/go`。
...
**Client：** Account（注册/登录/会话/偏好）、Databases（文档 CRUD + count）、Groups 与 Memberships、Realtime（WebSocket 订阅）、Assets、Payments、Subscriptions、Functions（`invokeFunction`）、Leaderboards（提交/me/top）、Analytics（`ingest` + 包根 `AnalyticsEventBuffer` 批量缓冲器）。

**Server：** Health、Projects、Users、Groups、Databases（库/集合/属性/索引/文档/Bulk）、API Keys、OAuthProviders、Storage（Bucket/File）、Functions、Payments、Assets、Subscriptions、Billing、Outbox、AuditLogs、Leaderboards（提交/条目/top/结算查询）、Analytics（ingest + 固定形状查询），以及 18 个 Agent 默认工具目录（`agentTools` / `lookupAgentTool`）。
...
## 版本策略与兼容承诺（A10 决策 memo，2026-09-05 成文；2026-09-06 维护者拍板：按推荐三项执行，决议回写 `docs/developer/15-exit-poc.md` A10）
...
**决议（2026-09-06，维护者，按推荐三项）**：① 0.2.0（minor 携带破坏性 + migration note）先行收拢分叉——**已于 2026-09-07 发布**（三面同 train，migration note 见 `CHANGELOG.md` 各 0.2.0 小节）；② 1.0.0 与转出门禁绑定、自此冻结契约（尚未触发）；③ 兼容矩阵 = 不承诺旧 SDK × 新服务端、服务端支持最近两个 SDK minor。
