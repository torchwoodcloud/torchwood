...
2. **平台侧(✅ 已实现,两项)**:
   - **孤儿数据面护栏(`815c528`)**:CreateProjectInternal 前置检查 `tw_<id>` schema 是否已存在,存在即 `FailedPrecondition` 拒绝并给出恢复指引(恢复控制面行保原 internal_id,或手动清理数据面),不再静默收养。集成测试复刻事故形态锁定。本次事故的精确命中场景:9-07 13:33 引导时 `tw_blog` 已存在,护栏会在部署时刻报错而非 30 小时后在业务路径炸开。
   - **internalIDCache TTL 重验(漂移分钟级暴露)**:此前租户号进程内缓存永不过期、仅项目删除路径失效——本次事故中漂移被 9-07 起长驻的旧进程遮蔽到今天重启才显形。现命中超过核验间隔(默认 30s)即回库比对:值漂移→切换新值并 WARN(含带外重置提示);行被带外删除→与首次解析同语义报错;瞬时故障→回退旧值保可用。每项目每 30s 至多一次额外 PK 点查。测试:`TestInternalIDCache_TTLReverify`/`StaleEntryOnMissingRow`。
