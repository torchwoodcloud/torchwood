...
> **迁移基线重定（2026-09-06）**：`db/migrations` 已从 33 个迁移重定为 5 个基线迁移（000001 控制面 / 000002 事件脊柱 / 000003 全局 catalog+账本 / 000004 RBAC·RLS·roles_sig / 000005 pgvector）；拍板依据 = 无生产存量库、全部环境重建。本文出现的历史迁移号按此映射、正文不改写：000025/000032→000003，000026/000027/000029/000031/000033→000004，000028→000002，000030→000005。
...
- **标识符治理（2026-09-04 落地形态；2026-09-06 勘误见下）**：逻辑 ID 与物理名解耦——**collectionID 字符集维持 `^[a-zA-Z_][a-zA-Z0-9_]*$` ≤40 不放宽**（原草图的 `[a-z0-9-]` ≤36 挂账待需求信号：snake_case 与属性键习惯一致，且避免 `_perms`/realtime 频道约定动荡）、属性 key ≤63、索引 ID ≤40；物理名 `c_<base32(8)>` 服务端分配（碰撞重试），截断/碰撞类缺陷机制性不可达；`_perms._collection` 与 realtime 频道保持逻辑 ID；**物理名不出现在任何 API 响应**（内部实现细节）。
- **【勘误 2026-09-06：随机物理名退役，物理表名 = collectionID】** 运维可读性裁决推翻本节 2026-09-04 形态：物理表名直接使用逻辑 collectionID（不加前缀），`c_<base32>` 随机分配与碰撞重试删除；**collectionID 收紧为小写 `^[a-z_][a-z0-9_]*$` ≤40**（psql/pg_dump 等运维路径免引号直用；原 `[a-z0-9-]` 放宽挂账就此关闭）。`catalog_collections.physical_name` 列保留为 collectionID 冗余投影；`resolvePhysicalTable` 点查与进程内缓存保留（语义收敛为存在性判定——行缺失 → NotFound，防 42P01 裸错误）。随解耦消失的"组合长度自然满足"红利同步归还：`idx_<coll>_<id>` 组合长度重新由 `validateIndexNameLen`（app 入口 + infra 二道防线）把守。**连带不变量**：跨项目/跨库同名集合合法（物理名唯一索引随勘误删除，唯一性由 catalog 主键一比一保证），凡按 `physical_name` 反查 catalog 的 SQL（RLS policy 子查询 `policyCatalogLookup`、`tw_set_document_acl` 白名单校验）必须按 (project, database) 三元组收窄，否则同名集合 >1 行触发 21000。"物理寻址字段不出现在任何 API 响应"不变量保留。
...
1. **物理独立性是重设计的根基**：§3.1 的全部演进自由——物理名服务端分配（`c_<base32>`）、全局 catalog、多集群分片出口、schema 演进状态机——都建立在"无人能依赖物理布局"之上。暴露 SQL 即永久冻结物理 schema，分片出口与演进契约同时报废。
   **【勘误 2026-09-06】** 物理表名 = collectionID 后本论点部分弱化：collectionID 本就是 API 可见且不可变的用户命名标识符，其作为表名进入"改名会破坏运维脚本"的约束集，与 schema 名 `tw_<p>_<db>` 同级（后者自始可读可依赖）——冻结面从"表名 + schema 名"收敛为"schema 名 + 表名 = 逻辑 ID"。API 层零物理寻址泄漏的不变量保留，AST 查询单栈不暴露表名，"无人能依赖物理布局"在 API/查询契约层面依然成立；弱化的仅是 DB 层面未来表名演进的自由（POC 无存量用户，此项自由从未被行使）。分片出口经 schema 粒度搬迁，不受表名影响。
