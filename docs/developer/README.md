...
> 2026-09-21 第二次全量重写：逐章按代码复核并重写为"描述现状"的规范文档——清偿 2026-09-16 全量重写后以增量补丁积累的分期演进叙述（期号 / 里程碑 / 版本演进史）与代码漂移。`authz-matrix.md` 为生成物（`mise run gen:authz-matrix`），不在重写范围；`15-exit-poc.md` 为门禁账本（只追加闭环证据），不做文风重写。

## 章节索引

| 章节 | 内容 | 适合读者 |
|------|------|----------|
| [01-overview.md](01-overview.md) | 架构总览：产品定位、技术栈、Clean 四层、目录树、五进程拓扑、数据面三层、典型调用链、关键设计机制 | 所有开发者 |
...
| [08-functions.md](08-functions.md) | 函数执行：dispatcher 唯一执行路径、常驻 runner 池、运行时指定与部署源（zip / git / BYO 镜像）、执行身份（`twx_` token）、触发器（HTTP / cron / 事件）、客户端调用面、多机部署 | 后端开发者 |
...
| [13-operations.md](13-operations.md) | 部署与运维：五进程形态、构建发布、生产配置要点（双账号契约、roles-sig 时序）、健康检查、规模预警线、备份恢复、vector / RBAC runbook | 运维 / 部署负责人 |
