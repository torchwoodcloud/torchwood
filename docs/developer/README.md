...
> 2026-09-16 全量重写：逐章按代码复核并重组成人类可读的规范文档。`authz-matrix.md` 为生成物（`task gen:authz-matrix`），不在重写范围。
...
| [04-codegen.md](04-codegen.md) | 代码生成与工具链：Task、Buf proto 生成、config 生成、Wire、生成顺序与漂移门禁、新增 RPC 生成侧清单 | 后端开发者 |
...
| [11-testing.md](11-testing.md) | 测试与质量：测试分层、testutil 集成库、`task test`、lint 全量门禁、CI 流水线、健康检查 | 所有开发者 |
