...
7. **查询全参数化 + 白名单**：事件名/`prop_key` 服务端校验后参数化（沿 `aggregateDocuments` 纪律），无字符串拼接值（设计稿要求的 HOUR 粒度可选 `timezone` 参数**尚未实现**——当前期边界恒 UTC，跟进项见设计稿 §4）；查询事务内 `SET LOCAL statement_timeout = '15s'`（慢查询显式报错不挂死）。护栏：HOUR 窗 ≤7 天、DAY 窗 ≤366 天、Breakdown ≤30 天、Retention cohort 窗 ≤92 天、ListUserEvents 窗 ≤92 天、Top-N ≤50。
