...
- **出处**：redesign §6 演进路径总览剩余挂账（"并行测试基建（SetupTestDB 争用 + 迁移循环集群级角色竞态：DROP ROLE 撞并行库中 tw_owner 对象，串行绿）"）；06-databases §12（`pkg/testutil/db.go:SetupTestDB`、`migrations_cycle_test.go`）。
