...
func fakeRunbookErr(code, msg string) error {
	return &runbookCallError{code: code, err: fmt.Errorf("rpc failed: %s", msg)}
}
...
func runUpOnWorld(t *testing.T, w *fakeRunbookWorld, dir string, mutate func(*runbookRunOptions)) (runbookUpSummary, error) {
	t.Helper()
	var out, errOut strings.Builder
	opts := runbookRunOptions{Dir: dir}
	if mutate != nil {
		mutate(&opts)
	}
	err := runRunbookUp(w.caller, opts, &out, &errOut)
	var summary runbookUpSummary
	if out.Len() > 0 {
...
func runDownOnWorld(t *testing.T, w *fakeRunbookWorld, dir string, mutate func(*runbookRunOptions)) (runbookDownSummary, error) {
	t.Helper()
	var out, errOut strings.Builder
	opts := runbookRunOptions{Dir: dir}
	if mutate != nil {
	...
	err := runRunbookDown(w.caller, opts, &out, &errOut)
	var summary runbookDownSummary
	if out.Len() > 0 {
		require.NoError(t, decodeJSON(out.String(), &summary))
	}
	return summary, err
...
func TestRunbookUpDryRun(t *testing.T) {
	...
	opts := runbookRunOptions{Dir: dir, DryRun: true}
	err := runRunbookUp(w.caller, opts, &out, &errOut)
...
func TestRunbookUpDryRunReportsWouldFail(t *testing.T) {
	...
	var out strings.Builder
	opts := runbookRunOptions{Dir: dir, DryRun: true}
	err := runRunbookUp(w.caller, opts, &out, &strings.Builder{})
...
func TestRunbookProgressLineContract(t *testing.T) {
	...
	require.NoError(t, runRunbookUp(w.caller, runbookRunOptions{Dir: dir}, &stdout, &stderr))
	...
	require.NoError(t, runRunbookDown(w.caller, runbookRunOptions{Dir: dir, All: true, Quiet: true}, &stdout2, &stderr2))
...
func TestRunbookUpToValidation(t *testing.T) {
	...
	t.Run("to 等于当前版 = no-op 退 0（CI 幂等，D14）", func(t *testing.T) {
		summary, err := runUpOnWorld(t, w, dir, func(o *runbookRunOptions) { o.To, o.ToSet = 2, true })
		require.NoError(t, err)
		require.Empty(t, summary.Applied)
	})

	t.Run("to 低于当前版 = UsageError", func(t *testing.T) {
		_, err := runUpOnWorld(t, w, dir, func(o *runbookRunOptions) { o.To, o.ToSet = 1, true })
	...
	t.Run("to 超出本地最大版 = UsageError", func(t *testing.T) {
		_, err := runUpOnWorld(t, w, dir, func(o *runbookRunOptions) { o.To, o.ToSet = 5, true })
		var ue *runbookUsageError
		require.ErrorAs(t, err, &ue)
	...
	t.Run("to 中途版本只应用到该版", func(t *testing.T) {
		...
		summary, err := runUpOnWorld(t, w2, dir, func(o *runbookRunOptions) { o.To, o.ToSet = 1, true })
		require.NoError(t, err)
		require.Equal(t, int64(1), summary.CurrentVersion)
		w2.mu.Lock()
		defer w2.mu.Unlock()
...
func TestRunbookDownAllThenUpCycle(t *testing.T) {
	...
	summary, err := runDownOnWorld(t, w, dir, func(o *runbookRunOptions) { o.All = true })
	require.NoError(t, err)
...
func TestRunbookDownIrreversible(t *testing.T) {
	...
	// dry-run 预报不可逆，仍退 0。
	var out strings.Builder
	err = runRunbookDown(w.caller, runbookRunOptions{Dir: dir, DryRun: true}, &out, &strings.Builder{})
	require.NoError(t, err)
	var summary runbookDownSummary
	require.NoError(t, decodeJSON(out.String(), &summary))
...
func TestRunbookDownToValidation(t *testing.T) {
	...
	t.Run("to 等于当前版 = UsageError", func(t *testing.T) {
		_, err := runDownOnWorld(t, w, dir, func(o *runbookRunOptions) { o.To, o.ToSet = 2, true })
		var ue *runbookUsageError
		require.ErrorAs(t, err, &ue)
	})

	t.Run("to 为负 = UsageError", func(t *testing.T) {
		_, err := runDownOnWorld(t, w, dir, func(o *runbookRunOptions) { o.To, o.ToSet = -1, true })
		var ue *runbookUsageError
		require.ErrorAs(t, err, &ue)
	})

	t.Run("to 与 all 同给 = UsageError", func(t *testing.T) {
		_, err := runDownOnWorld(t, w, dir, func(o *runbookRunOptions) { o.To, o.ToSet, o.All = 1, true, true })
		var ue *runbookUsageError
		require.ErrorAs(t, err, &ue)
	})

	t.Run("to 0 等价 all", func(t *testing.T) {
		summary, err := runDownOnWorld(t, w, dir, func(o *runbookRunOptions) { o.To, o.ToSet = 0, true })
		require.NoError(t, err)
		require.Equal(t, int64(0), summary.CurrentVersion)
	})
}
...
func runStatusOnWorld(t *testing.T, w *fakeRunbookWorld, dir string) runbookStatusOut {
	t.Helper()
	var out strings.Builder
	require.NoError(t, runRunbookStatus(w.caller, dir, &out))
	var res runbookStatusOut
...
func TestRunbookForgive(t *testing.T) {
	t.Run("从顶版逐个摘到 N，不动资源", func(t *testing.T) {
		...
		var out strings.Builder
		require.NoError(t, runRunbookForgive(w.caller, dir, 2, &out))
		var res runbookForgiveSummary
		require.NoError(t, decodeJSON(out.String(), &res))
		...
		require.NoError(t, runRunbookForgive(w.caller, dir, 1, &out2))
		var res2 runbookForgiveSummary
	...
	t.Run("未应用版本不可 forgive", func(t *testing.T) {
		w := newFakeRunbookWorld()
		dir := writeEngineFixtureDir(t)
		err := runRunbookForgive(w.caller, dir, 2, &strings.Builder{})
		require.Error(t, err)
	...
	t.Run("本地无该版本文件不可 forgive", func(t *testing.T) {
		...
		dir := writeEngineFixtureDir(t)
		applyFixtureFully(t, w, dir)
		err := runRunbookForgive(w.caller, dir, 5, &strings.Builder{})
		require.Error(t, err)
		require.Contains(t, err.Error(), "no local runbook file")
	})
	...
	t.Run("forgive 后重跑 up 以新 checksum 重放（注释级改动不触资源）", func(t *testing.T) {
		...
		require.NoError(t, runRunbookForgive(w.caller, dir2, 1, &strings.Builder{}))
...
func TestRunbookUpCmdUsageErrorSurface(t *testing.T) {
	...
	err := wrapRunbookUsageError(v, &runbookUsageError{err: errors.New("boom")})
