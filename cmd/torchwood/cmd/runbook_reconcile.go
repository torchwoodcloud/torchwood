...
// （D12 白名单 + 归一化）与逐动作的幂等 reconcile。本层只依赖 runbookCaller
...
// runbookActionReport 是一个（子）操作的进度与摘要报告。
type runbookActionReport struct {
...
func runbookReport(verb, target, result string) runbookActionReport {
	return runbookActionReport{Verb: verb, Target: target, Result: result}
}

func runbookSimpleOutcome(verb, target, result string) runbookActionOutcome {
	rep := runbookReport(verb, target, result)
	return runbookActionOutcome{Report: rep, Events: []runbookActionReport{rep}}
}
...
func (r *runbookReconciler) createCollection(body map[string]any) (runbookActionOutcome, error) {
	...
	events := []runbookActionReport{runbookReport("create_collection", target, result)}
