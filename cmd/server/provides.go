...
import (
	...
	"github.com/torchwoodcloud/torchwood/internal/app"
...
// realtime-subscriber → metrics → debug；逆序停止仅用于框架内部 Init/OnPreStart
...
// 可用。debug 垫底：pprof/version 诊断面关停期间无消费方，先停无影响。
// cleanup（DB/Redis 等底层资源）不注册进 Lynx，在 runner.RunE()
// 返回后由 main 统一执行。
func NewComponents(
