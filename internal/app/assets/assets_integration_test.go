...
func TestIntegration_PaidTopupGrantsInSameTx(t *testing.T) {
	...
	byDef, err := bunrepo.NewAssetHoldingRepository(db).ListByDef(ctx, projectID, domainassets.OwnerTypeUser, "", def.ID, 10, time.Now().Add(time.Hour), false)
	require.NoError(t, err)
	require.Len(t, byDef, 1)
	byDef, err = bunrepo.NewAssetHoldingRepository(db).ListByDef(ctx, projectID, domainassets.OwnerTypeUser, "u1", def.ID, 10, time.Now().Add(time.Hour), false)
	require.NoError(t, err)
	require.Len(t, byDef, 1)
	byDef, err = bunrepo.NewAssetHoldingRepository(db).ListByDef(ctx, projectID, domainassets.OwnerTypeUser, "u2", def.ID, 10, time.Now().Add(time.Hour), false)
	require.NoError(t, err)
