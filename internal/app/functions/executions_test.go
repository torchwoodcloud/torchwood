...
func TestCreateExecution_MetersFunctionDuration(t *testing.T) {
	...
	executor := newMockExecutor(&domainfunctions.ExecutionResult{StatusCode: 0, DurationMS: 42}, nil)
	meter := &recUsage{}
	uc := NewFunctionsWithUsage(&config.AppConfig{}, executor, repo, newMockQueue(), meter, nil, Semaphores{}, nil, nil, nil)
...
func TestRecoverOrphanExecutions_EnumeratesActiveProjectsWithBudget(t *testing.T) {
	...
	uc := NewFunctionsWithUsage(&config.AppConfig{}, newMockExecutor(nil, nil), repo, newMockQueue(), nil, projects, Semaphores{}, nil, nil, nil)
