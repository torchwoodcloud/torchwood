...
// reconcile 实现——全部经 Caller 抽象，不 import genproto。
...
func (r *runbookReconciler) bucketIDByName(name string) (string, error) {
	...
	if b == nil {
		return "", &CallError{code: runbookCodeNotFound, err: fmt.Errorf("bucket named %q not found", name)}
	}
...
func (r *runbookReconciler) updateAssetDef(body map[string]any) (runbookActionOutcome, error) {
	...
	if def == nil {
		return runbookActionOutcome{}, &CallError{code: runbookCodeNotFound, err: fmt.Errorf("asset def %q not found", code)}
	}
	req := make(map[string]any, len(body)+1)
	for k, v := range body {
		req[k] = v
