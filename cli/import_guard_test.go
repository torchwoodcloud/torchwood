...
// cliSourceRoots 是 CLI 源码的全部落点：实现主体（本包）与入口装配
// （cmd/torchwood 的 main.go/app.go），守卫一并覆盖。
var cliSourceRoots = []string{".", filepath.Join("..", "cmd", "torchwood")}
...
func TestNoProtoGRPCImports(t *testing.T) {
	for _, root := range cliSourceRoots {
		for _, v := range scanForbiddenImports(t, root) {
			t.Error(v)
		}
	}
}
