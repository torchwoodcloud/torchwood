...
func newDatabasesCollectionsUpdateCmd(g *GlobalFlags) *verb {
	...
	return newVerb(g, "update", "update a collection (only explicitly passed fields)", "databases collections update [--name] [--permissions] [--document-security] [--disabled] <database-id> <collection-id>",
		func(fs *flag.FlagSet) {
...
func newDatabasesAttributesCreateCmd(g *GlobalFlags) *verb {
	...
	return newVerb(g, "create", "create an attribute (types: string/integer/float/boolean/datetime)", "databases attributes create --key <key> --type <type> <database-id> <collection-id>",
		func(fs *flag.FlagSet) {
...
func newDatabasesDocumentsCreateCmd(g *GlobalFlags) *verb {
	...
	return newVerb(g, "create", "create a document (--data is the document data JSON object)", "databases documents create --data '{...}' <database-id> <collection-id>",
		func(fs *flag.FlagSet) {
...
func newDatabasesDocumentsUpdateCmd(g *GlobalFlags) *verb {
	...
	var version int64
	return newVerb(g, "update", "update a document (--version required; at least one of --data/--permissions/--increment)", "databases documents update --version <n> [--data] [--permissions] [--increment] <database-id> <collection-id> <document-id>",
		func(fs *flag.FlagSet) {
...
func newDatabasesDocumentsUpsertCmd(g *GlobalFlags) *verb {
	...
	return newVerb(g, "upsert", "upsert a document: update if the document-id exists, create otherwise", "databases documents upsert --data '{...}' --conflict-columns '[...]' <database-id> <collection-id> <document-id>",
		func(fs *flag.FlagSet) {
...
func newDatabasesDocumentsDeleteCmd(g *GlobalFlags) *verb {
	var version int64
	return newVerb(g, "delete", "delete a document (--version required, OCC on user collections)", "databases documents delete --version <n> <database-id> <collection-id> <document-id>",
		func(fs *flag.FlagSet) {
			fs.Int64Var(&version, "version", 0, "current document version (required for OCC on user collections, from the version field returned by get)")
		},
		func(v *verb, env *commands.Environment, args []string) error {
...
func newDatabasesDocumentsBulkUpdateCmd(g *GlobalFlags) *verb {
	...
	return newVerb(g, "bulk-update", "bulk update documents (sharing one set of data/permissions)", "databases documents bulk-update --document-ids '[...]' --data '{...}' <database-id> <collection-id>",
		func(fs *flag.FlagSet) {
...
func newDatabasesDocumentsBulkDeleteCmd(g *GlobalFlags) *verb {
	...
	return newVerb(g, "bulk-delete", "bulk delete documents", "databases documents bulk-delete --document-ids '[...]' <database-id> <collection-id>",
		func(fs *flag.FlagSet) {
			fs.StringVar(&documentIDs, "document-ids", "", "document IDs JSON array (required)")
		},
		func(v *verb, env *commands.Environment, args []string) error {
