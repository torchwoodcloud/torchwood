...
Docker + Compose and [mise](https://mise.jdx.dev/) (`curl https://mise.run | sh`); `mise install` provisions Go, Node, pnpm, buf, protoc(+protoc-gen-go) and golangci-lint at the versions pinned in `mise.toml`.
...
```bash
mise run db:migrate   # run with the bootstrap (superuser) DSN — see §4.5
```

### 4. Toolchain & codegen

```bash
mise install   # go, node, pnpm, buf, protoc, protoc-gen-go, golangci-lint (first time)
mise run generate:all    # buf generate + config proto + wire:all
mise run console:install # pnpm install (first time)
```

### 5. Build & run

```bash
mise run build        # console:build then server/worker/dispatcher/packer/torchwood -> ./bin/
./bin/server      # or ./bin/server.exe on Windows
# dev mode:
mise run dev:server   # go run ./cmd/server
```

Modify Console code? `mise run console:build` before `mise run build` — the SPA is `//go:embed`'d.
...
| Command | Purpose |
|------|---------|
| `mise run docker:up` / `docker:down` / `docker:purge` | start / stop / wipe local infra |
| `mise run db:migrate` | run `db/migrations` |
| `mise run generate:proto` / `generate:config` / `wire:all` / `generate:all` | buf / config proto / Wire |
| `mise run console:install` / `console:build` / `console:dev` | frontend pnpm workflow |
| `mise run dev:server` / `mise run dev:worker` | run server/worker without building |
| `mise run build` | console + server + worker + dispatcher + packer + `bin/torchwood` CLI |
| `mise run test` | SDK Go/TS tests + `go test -v ./... -cover` |
...
├── cmd/torchwood/          # Torchwood CLI entry (assembly only)
├── cli/                 # Torchwood CLI implementation (lynx-go/commands, sdk/go InvokeJSON)
...
- **Wire DI**: `cmd/server/provides.go` declares the provider set, `cmd/server/wire_gen.go` (and `cmd/worker` equivalent) is generated by `mise run wire:all`. Re-run after provider changes.
- **Three processes**: `server` (gRPC + gateway + custom HTTP handlers + metrics + embedded Console), `worker` (function execution queue consumer, independent Wire), `CLI` (`bin/torchwood`, implementation in `cli/`, assembly in `cmd/torchwood`; lynx-go/commands over `sdk/go/server` `InvokeJSON`; no direct `genproto` import — `rpc` escape hatch covers all new RPCs).
- **Three-tier storage**: `public` control plane & event spine (`projects`, `admins`, `api_keys`, `audit_logs`, `outbox`/`outbox_dead`, global catalog tables `catalog_databases`/`catalog_collections`, via bun + golang-migrate); `tw_<project.id>` project plane — system static tables (`users`, `sessions`, `identities`, `groups`, `memberships`, `buckets`, `files`) + ledger/functions/OAuth (`internal/infra/projectschema/`); `tw_<project.id>_<database.id>` business plane — user collections only (real tables named by collectionID, `_tenant` + embedded `_acl` + RLS policies).
- **API surface**: Protobuf is the single source of truth (`proto/` → `genproto/`). REST via grpc-gateway; file multipart and OAuth callbacks via `internal/api/serverhttp`. gRPC methods require `method_auth` annotations (collected into a policy registry at startup).
- **Auth**: end-user JWT/session cookie, API Key (participates as `keys` + `key:<id>` roles in `_acl`, not a bypass), console admin JWT (`TORCHWOOD_session_console` HttpOnly cookie). Admins target a project via `X-Torchwood-Project`.
...
Integration tests (`internal/infra/documentdb/postgres_test.go`, `internal/app/client/account_test.go`, etc.) auto-create/drop the `TORCHWOOD_test` DB. DSNs come from `TORCHWOOD_TEST_DATABASE_SOURCE` / `TORCHWOOD_TEST_ADMIN_DATABASE_SOURCE` (in `.env.example`); `mise run test` loads `.env` automatically.
...
| [06-databases](docs/developer/06-databases.md) | dynamic documents, `_acl` + RLS permissions, query AST |
